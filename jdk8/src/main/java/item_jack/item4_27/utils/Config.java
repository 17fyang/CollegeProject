package item_jack.item4_27.utils;
...
public class Config {
	...
	private List<List<Record>> lineList;//每一行的数据源（不去重）
	...
	public OriginalDataList getOriginalData(File dataDeal_source,int number) throws Exception{
		...
		List<List<Record>> lineRecord=new ArrayList<List<Record>>();//行数据的容器
	...
	public List<List<Record>> getLineList() {
		List<List<Record>> temp=this.lineList;
		this.lineList=null;
		return temp;
	}
...
class OriginalDataList{
	public OriginalDataList(Record[] sourceList,List<List<Record>> lineList) {
		this.lineList=lineList;
		this.sourceList=sourceList;
	}
	Record[] sourceList;//dataDeal数据源
	List<List<Record>> lineList;//每一行的数据源（不去重）
}
