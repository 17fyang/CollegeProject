...
public class CLS {
	...
	public double[] decode(String single){
		...
		double[] x = {-1,-1};
		x[0] = a * (6.0 - 0) / (Math.pow(2, ModelConfig.geneNumber/2) - 1);	//x的基因
		x[1] = b * (6.0 - 0) / (Math.pow(2, ModelConfig.geneNumber/2) - 1);	//y的基因
		return x;
	}
	...
	private double formula(double[] x) {
		int j=2;
		double fitness = Math.sin(j * x[0]) * Math.sin(j * x[0]) 
				+ Math.sin(j * x[1]) * Math.sin(j * x[1]);
		return fitness;
	}
