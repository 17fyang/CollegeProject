...
public class Data {
	...
	public Data(String dataUrl,int neuronLength) {
		...
		try {
			...
			testList=splitList(dataList,1);//测试集
		} catch (Exception e) {
	...
	public  List<Flower> init(String dataUrl) throws Exception {
		...
		for(int i=0;i<oneData.length;i++) {
			...
			int type=0;
			switch(detail[detail.length-1]) {
			case "win" :type=0;break;
			case "tie" :type=1;break;
			case "loss" :type=2;break;
			}
		...
		this.dataLength=list.get(0).getAttributeList().size();
		in.close();
		return list;
	}
	...
	//105个训练数据，45个测试数据
		public List<Flower> splitList(List<Flower> dataList, int type) {
			List<Flower> list=new LinkedList<Flower>();
			int trainNumber=Main.trainNumber;
			for(int i=0;i<dataList.size();i++) {
				if(type==0 && i<trainNumber)	list.add(dataList.get(i));
				if(type==1 && i>=trainNumber)	list.add(dataList.get(i));
			}
			return list;
		}
